        ...
        foreach($invites as $inv) {
            ...
            if (empty($grpinv)) { //if empty, then this group doesn't exist so delete the invite!
                ...
                $returntext .= '<div class="team-invite"><span class="team-groupname">'.$grpinv->name.'</span> '.
                               '<a href="'.$CFG->wwwroot.'/blocks/teams/manageteam.php?id='.$theBlock->instance->id.'&groupid='.$inv->groupid.'&action=accept">'.get_string('accept','block_teams').'</a> | '.
                               '<a href="'.$CFG->wwwroot.'/blocks/teams/manageteam.php?id='.$theBlock->instance->id.'&groupid='.$inv->groupid.'&action=decline">'.get_string('decline','block_teams').'</a>';
               $returntext .= '</div>';
            }
        }
    } else {
...
function teams_new_group_form(&$theBlock) {
    ...
    $str = '<form action="'.$CFG->wwwroot.'/blocks/teams/manageteam.php" method="post">';
